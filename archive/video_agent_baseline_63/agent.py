...
class EvalStatistics:
    ...
    def update(self, result: Dict):
        ...
        # Track validity based on is_valid field (whether fallback was used)
        # is_valid=False means the result came from fallback mechanism
        if not is_valid_flag:
            self.invalid_videos += 1
            self.fallback_videos += 1
            self.case_types["INVALID"] += 1
            # Still count as processed but mark as invalid
            # Don't return early - we still want to track if the fallback happened to be correct
        
        # Check if prediction is valid (should always be true with fallback mechanism)
        has_valid_prediction = predicted_int != -1
        is_correct = predicted_int == label and has_valid_prediction
        
        # If prediction parsing failed (shouldn't happen with fallback), skip further processing
        if not has_valid_prediction:
            if is_valid_flag:  # Only count as invalid if not already counted
                self.invalid_videos += 1
                self.case_types["INVALID"] += 1
            return
...
class VideoAgent:
    ...
    def _save_global_results(self, results: List[Dict], output_dir: str):
        ...
        for result in results:
            ...
            # Determine case type based on is_valid flag
            if not is_valid_flag:
                case_type = "INVALID"
